...
public class ShowMapTitle2 : MonoBehaviour
{
    ...
    IEnumerator ShowMapTitle2_Coroutine()   // 텍스트의 컬러값 중 a값을 0에서 1로 올리는 함수 
    {
        ...
        while (mapTitle2.color.a != 1f) // 1이 되기 전까지 반복 
        {
    ...
    IEnumerator CloseMapTitle2_Coroutine() // 텍스트의 컬러값 중 a값을 255에서 0으로 내리는 함수 
    {
        ...
        while (mapTitle2.color.a > 0f) // 0이 되기 전까지만 반복 
        {
